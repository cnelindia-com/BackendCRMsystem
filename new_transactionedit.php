...
$check_already_confirm_sql = "SELECT confirmed FROM transactions WHERE id = '$edit_id'";
...
{ 
	...
	if($confirmed == 0){
		...
		if($hwe_transaction_type_chek == 'Withdrawal'){
			$update_approve_status = "UPDATE transactions SET status ='$status',amount='$amount',confrimedby='$user',withdrawal_bank_id='$Banktype_deposit',confirmeddate='$confirmdate', confirmed = '1', remark = '$comment', reject_reason = '$reject_reason' where id=".$_GET['edit_id'];
		}
		else{
			$update_approve_status = "UPDATE transactions SET status ='$status',amount='$amount',confrimedby='$user',bankid='$Banktype_deposit',confirmeddate='$confirmdate', confirmed = '1', remark = '$comment', reject_reason = '$reject_reason' where id=".$_GET['edit_id'];
			
		}
	}
	else{
		if($hwe_transaction_type_chek == 'Withdrawal'){
			$update_approve_status = "UPDATE transactions SET status ='$status',amount='$amount',confrimedby='$user',withdrawal_bank_id='$Banktype_deposit', remark = '$comment', reject_reason = '$reject_reason' where id=".$_GET['edit_id'];
		}
		else{
			$update_approve_status = "UPDATE transactions SET status ='$status',amount='$amount',confrimedby='$user',bankid='$Banktype_deposit', remark = '$comment', reject_reason = '$reject_reason' where id=".$_GET['edit_id'];
...
{ 
	...
	if($confirmed == 0){
		if($hwe_transaction_type_chek == 'Withdrawal'){
			$update_approve_status = "UPDATE transactions SET status ='$status',amount='$amount',confrimedby='$user',withdrawal_bank_id='$Banktype_deposit',confirmeddate='$confirmdate', remark = '$comment', reject_reason = '$reject_reason' where id=".$_GET['edit_id'];
		}
		else{
			$update_approve_status = "UPDATE transactions SET status ='$status',amount='$amount',confrimedby='$user',bankid='$Banktype_deposit',confirmeddate='$confirmdate', remark = '$comment', reject_reason = '$reject_reason' where id=".$_GET['edit_id'];
...
if(isset($_GET['newedit_id1'])){
	echo "sourabh";	
	$neweditid= $_GET['edit_id'];
	$newedit_id1= $_GET['newedit_id1'];
	echo "SELECT * FROM `transactions` WHERE id='$newedit_id1' AND (transactiontype='Deposit' OR transactiontype='Bonus')";
	
	$transresult=mysqli_query($conn, "SELECT * FROM `transactions` WHERE id='$newedit_id1' AND (transactiontype='Deposit' OR transactiontype='Bonus')");
	
	$transrow= mysqli_fetch_assoc($transresult);

	if(($transrow['transactiontype'] !== 'Deposit')){
		echo "Deposit";
		$newamount=$transrow['amount'];		
		mysqli_query($conn, "UPDATE `transactions` SET bonus_amount='$newamount' WHERE id='$neweditid'");
		echo "UPDATE `transactions` SET amount='$newamount' WHERE id='$neweditid1'";
		...
		$newamount=$transrow['amount'];		
		mysqli_query($conn, "UPDATE `transactions` SET bonus_amount='$newamount' WHERE id='$neweditid'");
		echo "UPDATE `transactions` SET amount='$newamount' WHERE id='$neweditid'";
		$get_transaction = mysqli_query($conn,"SELECT * from transactions where id='$neweditid'");			
	}
...
<div class="container">
	<center><h2><strong>Transaction Details - <?php echo $get_accountrow['username']; ?>  < New Transaction> </strong></h2></center>
     <?php 
	if(!empty($success))
	{
	?>
	...
	?>
	<form class="form-horizontal" method="post" id="new_transactioneditform">
        <div class="container-fluid">
                ...
                <div class="col-sm-12">
                    <fieldset style="border: solid 1px #a0a0a0; padding: 2px 30px;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="amount" value="<?php echo $get_transaction_rows['amount']; ?>">
							</div>
                        </div>
						...
						?>
							<div class="form-group">
								...
								?>
								<div class="col-sm-8"><a style="text-decoration: underline;color:#0055ff;" href="javascript:void();" onclick="openwindow('approve_reject_transaction.php?edit_id=<?php echo $get_transaction_rows['bonus_promotion_id'];?>');"><? echo $get_transaction_rows['bonus_promotion_id']; ?></a>
								</div>
                                <?php																											
									}
								?>
							...
							<?php
							$bonus_trans_id = $get_transaction_rows['bonus_promotion_id'];
							$sql_bonus_tr = "SELECT amount FROM transactions WHERE id = '$bonus_trans_id'";
                        ...
                        <div class="form-group">
							...
							<div class="form-group">
                                ...
                                <?php
									...
									?>
									<div class="col-sm-8"><a style="text-decoration: underline;color:#0055ff;" href="javascript:void();" onclick="openwindow('approve_reject_transaction.php?edit_id=<?php echo $get_transaction_rows['deposit_transaction_id']; ?>');"><? echo $get_transaction_rows['deposit_transaction_id']; ?></a>
									</div>
									<?php																											
									}
								?>
            ...
            <div class="row" style="display: unset;">
            	<div class="btn_bar1" style="bottom:0px;text-align: center;">
                    <input class="btn1" type="submit" name="reject" value="Reject">&nbsp;
					<input class="btn1" type="submit" name="approve" value="Approve">&nbsp;
