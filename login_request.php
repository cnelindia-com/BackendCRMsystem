...
<div id="sc8" class="tabcontent" style="display: block;">
    ...
    <span>|</span>
    <a href="login_request.php" id="sc8_1" class="actives">Login Request</a>
    <span>|</span>
  ...
  <div class="filternav">
	<h2>Login Request</h2>
		<form method="post">
			...
			<div>
				<input type="button" class="btn btn-secondary" name="submit" id="submit" value="Search"/>
        ...
        <table class="table table-hover table-bordered results" id="table2excel">
            <thead style="background-color: #ff720b;">
                <tr>               
                 ...
                 <!-- <th class="col-md-3 col-xs-3">Finger Print Code</th>
                  <th class="col-md-3 col-xs-3">New Finger Print Code</th>-->
                  <th class="col-md-3 col-xs-3">Request Date</th>
                  <th class="col-md-3 col-xs-3">Admin User Bypass Status</th>
            ...
            <?php 
				...
				$login_reqsql=mysqli_query($conn, "SELECT * FROM `user_info`");
				$count=0;
				while($login_reqrows=mysqli_fetch_assoc($login_reqsql))
				{
			...
			?>
            	...
            	<tr>
                  ...
                  <td><?php echo $login_reqrows['user_id'];?></td>
                  
                  <?php
                  	$name=explode(" ", $login_reqrows['name']);
					$username=ucfirst(implode(", ", $name));
				  ?>
                  
                  <td><?php echo $username;?></td>
                  <td><?php echo $login_reqrows['type'];?></td>
                 <!-- <td></td>
                  <td></td>-->
                  <td><?php echo $login_reqrows['last_login'];?></td>
                  <?php
                  	if($login_reqrows['status']=='active')
					{
				  ?>
                  <td><?php echo 'TRUE';?></td>
                  <?php
				  ...
				  ?>
                  <td><?php echo $login_reqrows['status'];?></td>
                  <td><i class="fa fa-close ml-2"></i></td>
              	</tr>
            <?php
				}
			?>
