...
if(isset($_GET['delete_id']))
{
	...
	$dlt_sql = "DELETE from vip_setting where id=$delete_id";
	if(mysqli_query($conn,$dlt_sql)==TRUE)
	{
		$success='VIP Deleted Successfully.';
	}
}
/*
$user=$_SESSION['user'];
...
<div id="sc8" class="tabcontent" style="display: block;">
	<a href="customer.php" id="sc8_1">Account</a>
    <span>|</span>
  ...
  <div class="filternav">
	...
	<form name="spbFormTop" id="spbFormTop" method="post">
        ...
        <table>
            <tbody>
                <tr>
                    <th> 
                        ...
                        <a href="add_vip.php"  class="btn1">NEW</a>
            ...
            <tbody>       
          		<?php 
				$get_vip = mysqli_query($conn,"Select * from vip_setting");
				if(mysqli_num_rows($get_vip)>0)
				{
					$count=1;
					while($rowvip = mysqli_fetch_assoc($get_vip))
					{
						?>
						 <tr class="noExl">
                              <td><?php echo $count++;?></td>
                              <td><?php echo $rowvip['name'];?></td>
                              <td><?php echo $rowvip['description'];?></td>
                              <td><?php echo $rowvip['totaldeposit'];?></td>
                              <?php 
								if($rowvip['status']=="Active")
								{
								?>
								<td style="background: #C6FFD9;text-align: center;"><?php echo $rowvip['status'];?></td>  
								<?php
								}
								else if($rowvip['status']=="Suspended")
								{
								?>  
								<td style="background: #FFCCCC;text-align: center;"><?php echo $rowvip['status'];?></td>    
								<?php
								}                  
								?>
                              <td><a href="add_vip.php?edit_id=<?php echo $rowvip['id'];?>"><i class="fa fa-search ml-2"></i></a>
                              
                              <a href="?delete_id=<?php echo $rowvip['id'];?>" onclick="return confirm('Are you sure you want to delete?');"><i class="fa fa-trash ml-2"></i></a>
                              </td>                  
                         </tr>       
						<?php 
					}
